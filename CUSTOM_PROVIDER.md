...
AI Gateway 现在支持添加自定义服务商，让用户可以集成任何兼容 OpenAI API 格式的 AI 服务。用户可以自由填写服务商名称。

## 功能特性

1. **自定义服务商名称**: 支持用户自由填写任意服务商名称
...
5. **灵活配置**: 支持标准服务商（OpenAI、Anthropic、Gemini）和完全自定义的服务商
...
3. 点击"+ 添加配置"
4. 选择提供商类型：
   - **标准服务商**: OpenAI、Anthropic、Gemini（自动填充默认URL）
   - **自定义服务商**: 需要填写服务商名称
5. 对于自定义服务商，填写以下信息：
   - **服务商名称**: 自定义的服务商名称，如 `MyAIService`、`LocalLLM`、`CompanyAI`（只能包含字母、数字、连字符、下划线和点）
...
自定义服务商名称必须遵循以下规则：
- 长度：1-50个字符
- 允许的字符：字母（a-z, A-Z）、数字（0-9）、连字符（-）、下划线（_）、点（.）
- 示例：
  - ✅ `MyAIService`
  - ✅ `local-llm`
  - ✅ `company_ai_2024`
...
2. **自定义查找**: 如果不匹配预定义模式，系统会在用户配置的所有非标准 provider 中查找匹配的模型代码
3. **服务商识别**: 找到匹配的模型后，系统使用该配置的服务商名称进行后续处理
4. **协议路由**: 根据配置的协议格式将请求路由到相应的适配器
5. **响应转换**: 适配器处理请求并返回标准格式的响应

## 注意事项

1. **模型代码唯一性**: 确保不同自定义 provider 中的模型代码不重复，否则会使用第一个匹配的配置
2. **服务商名称唯一性**: 每个用户的服务商名称应该是唯一的，避免冲突
3. **API 兼容性**: 自定义 provider 必须兼容所选协议的 API 格式
4. **安全性**: API 密钥会自动加密存储
5. **默认配置**: 每个服务商名称只能有一个默认配置
6. **命名规范**: 服务商名称只能包含字母、数字、连字符、下划线和点
